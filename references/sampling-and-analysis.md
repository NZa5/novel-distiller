...
```bash
python scripts/corpus_index.py sample work/corpus-index.jsonl --output work/sampling-ledger.json --budget <B> --holdout-ratio 0.2 --seed 20260831
```
...
把 `A` 定义为可用于建模的非留出文本块数，`N` 定义为作品数，`B` 才是 `--budget` 的精读块数。用户没有另设限制时：`A <= 24` 则 `B=A`；否则 `B=min(A, max(24, min(80, 6*N)))`。例如两部作品有 100 个可用块，默认精读 24 块；十部作品有 100 个可用块，默认精读 60 块。预算不包含留出块，可以分批完成，不代表总索引块数。作品通过轮转获得近似等额机会，六类语义元数据通过欠覆盖优先评分参与选择。该算法只根据清单中的离散标签平衡覆盖；若仍有缺口，使用语义检索定向补读并记录理由。
