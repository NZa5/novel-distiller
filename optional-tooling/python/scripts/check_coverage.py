...
TARGETS = ("loaders/epub_security.py", "utils/prompt_safety.py", "utils/safe_text.py")

def main(path: str) -> int:
    data = json.loads(Path(path).read_text(encoding="utf-8"))
    files = data.get("files", {})
    selected = [v for k, v in files.items() if any(k.replace('\\', '/').endswith(t) or t in k.replace('\\', '/') for t in TARGETS)]
    statements = sum(int(v.get("summary", {}).get("num_statements", 0)) for v in selected)
    covered = sum(int(v.get("summary", {}).get("covered_lines", 0)) for v in selected)
